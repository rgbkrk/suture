...
import pytest
from cursor_controller import CursorPosition, CursorType, StreamingEditController
...
class CursorTracker:
    ...
    def assert_sequence(self, expected_sequence):
        ...
        assert len(self.positions) == len(expected_sequence), (
            f"Expected {len(expected_sequence)} cursor moves, got {len(self.positions)}"
        )
...
async def test_simple_typo_fix():
    ...
    expected = [
        ...
        CursorPosition(
            position=20, cursor_type=CursorType.SELECTION, selection_length=4
        ),
...
async def test_rice_crispy_squares():
    ...
    expected = [
        ...
        CursorPosition(
            position=13, cursor_type=CursorType.SELECTION, selection_length=6
        ),
...
async def test_deletion_only():
    ...
    expected = [
        ...
        CursorPosition(
            position=15, cursor_type=CursorType.SELECTION, selection_length=1
        ),  # ": 1"
        CursorPosition(
            position=15, cursor_type=CursorType.SELECTION, selection_length=10
        ),  # "0" completes it
    ]
...
async def test_regex_simple_typo():
    ...
    controller = StreamingEditController(
        document_text=document_text, on_cursor_move=tracker.on_cursor_move
    )
    ...
    for i, pos in enumerate(tracker.positions):
        print(
            f"  {i}: position={pos.position}, type={pos.cursor_type.value}, selection={pos.selection_length}"
        )
    ...
    expected = [
        ...
        CursorPosition(
            position=8, cursor_type=CursorType.SELECTION, selection_length=2
        ),  # Bug: should be 4
