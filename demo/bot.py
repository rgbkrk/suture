...
class CollaborativeBot:
    ...
    async def broadcast_cursor(
        self, position: int, cursor_type: str = "ai", anchor: Optional[int] = None
    ):
    ...
    async def _handle_cursor_move(self, cursor_pos: CursorPosition) -> None:
        ...
        if cursor_pos.cursor_type == CursorType.LOOKAHEAD:
            ...
            print(
                f"  📍 Selection from {anchor} to {position} (length: {cursor_pos.selection_length})"
            )
    ...
    async def stream_edit(self, current_text: str) -> bool:
        ...
        try:
            ...
            controller = StreamingEditController(
                document_text=current_text, on_cursor_move=self._handle_cursor_move
            )
...
async def main():
    if len(sys.argv) < 2:
        print(
            "Usage: uv run bot.py <doc_id> [--once] [--model MODEL] [--debug] [--wait SECONDS]"
        )
        print("\nExample:")
    ...
    bot = CollaborativeBot(
        doc_id, model=model, once=once, debug=debug, wait_time=wait_time
    )
    await bot.run()
