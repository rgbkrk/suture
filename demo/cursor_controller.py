...
class CursorPosition:
    ...
    selection_length: int = (
        0  # If > 0, shows selection from position to position+length
    )
...
class StreamingEditController:
    ...
    async def feed_token(self, token: str) -> None:
        ...
        try:
            ...
            if "replacement" in partial_data and self.last_match_position is not None:
            ...
            if self.last_match_position is None:
                pattern_match = re.search(
                    r'"pattern"\s*:\s*"([^"]*)"', self.accumulated_json
                )
                if pattern_match:
                    try:
                        ...
                        if match:
                            ...
                            await self._move_cursor(
                                CursorPosition(
                                    position=position, cursor_type=CursorType.LOOKAHEAD
                                )
                            )
                    except re.error:
                        pass
